...
#   rf <- ranger(dependent.variable.name = "Species", data = dat, write.forest = TRUE, probability = TRUE, activate.ts = TRUE, block.size = 2)
...
#   expect_silent(ranger(Species ~ ., data = dat, activate.ts = TRUE, block.size = 2))
# })
# 
# test_that("Error if sample fraction is 0 or >1", {
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = 0, activate.ts = TRUE, block.size = 2))
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = 1.1, activate.ts = TRUE, block.size = 2))
# })
# 
# test_that("Error if sample fraction is vector for regression", {
#   expect_error(ranger(Sepal.Length ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 0.2), activate.ts = TRUE, block.size = 2), 
...
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 0.2), activate.ts = TRUE, block.size = 2), 
...
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 1.1, 0.3), activate.ts = TRUE, block.size = 2), 
#                "Error: Invalid value for sample\\.fraction. Please give a value in \\(0,1\\] or a vector of values in \\[0,1\\]\\.")
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(-3, 0.5, 0.3), activate.ts = TRUE, block.size = 2), 
#                "Error: Invalid value for sample.fraction. Please give a value in \\(0,1] or a vector of values in \\[0,1\\]\\.")
# })
# 
# test_that("Error if sum of sample fraction vector is 0", {
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(0, 0, 0), activate.ts = TRUE, block.size = 2), 
...
#                       replace = FALSE, keep.inbag = TRUE, activate.ts = TRUE, block.size = 2), 
...
#                        replace = TRUE, keep.inbag = TRUE, activate.ts = TRUE, block.size = 2))
# })
# 
# test_that("Error if sample.fraction and case.weights", {
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.2, 0.3, 0.4), 
#                       case.weights = rbinom(nrow(iris), 1, 0.5), activate.ts = TRUE, block.size = 2), 
...
#   ## With replacement
#   rf <- ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.2, 0.3, 0.4), 
#                replace = TRUE, keep.inbag = TRUE, probability = TRUE, activate.ts = TRUE, block.size = 2)
#   inbag <- do.call(cbind, rf$inbag.counts)
#   expect_equal(unique(colSums(inbag[1:50, ])), 30)
#   expect_equal(unique(colSums(inbag[51:100, ])), 45)
#   expect_equal(unique(colSums(inbag[101:150, ])), 60)
...
#   ## Without replacement
#   rf <- ranger(Species ~ ., iris, num.trees = 5, sample.fraction = c(0.1, 0.2, 0.3), 
#                replace = FALSE, keep.inbag = TRUE, probability = TRUE, activate.ts = TRUE, block.size = 2)
#   inbag <- do.call(cbind, rf$inbag.counts)
#   expect_equal(unique(colSums(inbag[1:50, ])), 15)
#   expect_equal(unique(colSums(inbag[51:100, ])), 30)
#   expect_equal(unique(colSums(inbag[101:150, ])), 45)
...
#   expect_silent(ranger(as.factor(y) ~ ., data = dt, num.trees = 5, write.forest = TRUE, activate.ts = TRUE, block.size = 2))
# })
# 
...
#                holdout = TRUE, keep.inbag = TRUE, activate.ts = TRUE, block.size = 2)
...
#   expect_error(ranger(Species ~ ., iris, num.trees = 5, importance = "permutation", holdout = TRUE, activate.ts = TRUE, block.size = 2))
# })
# 
...
#   rf <- ranger(y ~ ., dat, num.trees = 1, activate.ts = TRUE, block.size = 2)
...
#   n <- 50
#   dat <- data.frame(y = factor(rbinom(n, 1, .5)), x = rnorm(n))
#   rf <- ranger(y ~ ., dat, num.trees = 1, probability = TRUE, activate.ts = TRUE, block.size = 2)
...
#   dat[4, 5] <- NA
#   dat[25, 1] <- NA
#   expect_error(ranger(Species ~ ., dat, num.trees = 5, activate.ts = TRUE, block.size = 2), "Missing data in columns: Species, Sepal.Length")
# })
# 
# test_that("No error if missing value in irrelevant column, training", {
#   dat <- iris
#   dat[1, "Sepal.Width"] <- NA
#   expect_silent(ranger(Species ~ Sepal.Length, dat, num.trees = 5, activate.ts = TRUE, block.size = 2))
# })
# 
# test_that("No error if missing value in irrelevant column, prediction", {
#   rf <- ranger(Species ~ Sepal.Length, iris, num.trees = 5, activate.ts = TRUE, block.size = 2)
#   dat <- iris
#   dat[1, "Sepal.Width"] <- NA
#   expect_silent(predict(rf, dat))
# })
...
#   rf <- ranger(y ~ x, dat, num.trees = 10, activate.ts = TRUE, block.size = 2)
...
#   dat <- data.frame(y = rbinom(100, 1, .5), x = rbinom(100, 1, .5))
#   rf <- ranger(y ~ x, dat, num.trees = 10, splitrule = "maxstat", alpha = 1, activate.ts = TRUE, block.size = 2)
#   split_points <- mapply(function(varID, value) {
#     value[varID > 0]
#   }, 
#   rf$forest$split.varIDs, 
...
#   dat <- data.frame(y = factor(rbinom(100, 1, .5)), x = rbinom(100, 1, .5))
#   rf <- ranger(y ~ x, dat, num.trees = 10, probability = TRUE, activate.ts = TRUE, block.size = 2)
#   split_points <- mapply(function(varID, value) {
#     value[varID > 0]
#   }, 
#   rf$forest$split.varIDs, 
...
#   rf <- ranger(Surv(time, status) ~ x, dat, num.trees = 10, splitrule = "logrank", activate.ts = TRUE, block.size = 2)
#   split_points <- mapply(function(varID, value) {
#     value[varID > 0]
#   }, 
#   rf$forest$split.varIDs, 
...
#   dat <- data.frame(time = runif(100, 1, 10), status = rbinom(100, 1, .5), x = rbinom(100, 1, .5))
#   rf <- ranger(Surv(time, status) ~ x, dat, num.trees = 10, splitrule = "C", activate.ts = TRUE, block.size = 2)
#   split_points <- mapply(function(varID, value) {
#     value[varID > 0]
#   }, 
#   rf$forest$split.varIDs, 
...
#   dat <- data.frame(time = runif(100, 1, 10), status = rbinom(100, 1, .5), x = rbinom(100, 1, .5))
#   rf <- ranger(Surv(time, status) ~ x, dat, num.trees = 10, splitrule = "maxstat", alpha = 1, activate.ts = TRUE, block.size = 2)
#   split_points <- mapply(function(varID, value) {
#     value[varID > 0]
#   }, 
#   rf$forest$split.varIDs, 
...
#   rf <- ranger(Species ~ ., dat, num.trees = 5, activate.ts = TRUE, block.size = 2)
#   expect_silent(predict(rf, dat))
# })
# 
# test_that("Prediction error not NA if oob.error=TRUE", {
#   rf <- ranger(Species ~ ., iris, num.trees = 5, activate.ts = TRUE, block.size = 2)
#   expect_false(is.na(rf$prediction.error))
#   
#   rf <- ranger(Surv(time,status) ~ ., veteran, num.trees = 5, activate.ts = TRUE, block.size = 2)
#   expect_false(is.na(rf$prediction.error))
# })
# 
# test_that("Prediction error is NA if oob.error=FALSE", {
#   rf <- ranger(Species ~ ., iris, num.trees = 5, oob.error = FALSE, activate.ts = TRUE, block.size = 2)
#   expect_true(is.na(rf$prediction.error))
#   
#   rf <- ranger(Surv(time,status) ~ ., veteran, num.trees = 5, oob.error = FALSE, activate.ts = TRUE, block.size = 2)
#   expect_true(is.na(rf$prediction.error))
# })
# 
...
#   
#   # Depth 1
#   rf <- ranger(Species ~ ., iris, num.trees = 5, max.depth = 1, activate.ts = TRUE, block.size = 2)
#   expect_true(all(forest_depth(rf) <= 1))
#   
#   # Depth 4
#   rf <- ranger(Species ~ ., iris, num.trees = 5, max.depth = 4, activate.ts = TRUE, block.size = 2)
...
#   rf <- ranger(y ~ ., dat, num.trees = 5, min.node.size = 1, max.depth = max.depth, activate.ts = TRUE, block.size = 2)
#   expect_true(all(forest_depth(rf) <= max.depth))
# })
# 
# test_that("Tree depth 0 equivalent to unlimited", {
#   set.seed(200)
#   rf1 <- ranger(Species ~ ., iris, num.trees = 5, max.depth = 0, activate.ts = TRUE, block.size = 2)
#   
#   set.seed(200)
#   rf2 <- ranger(Species ~ ., iris, num.trees = 5, activate.ts = TRUE, block.size = 2)
#   
...
#   rf <- ranger(Sepal.Length ~ ., iris, max.depth = 1, num.trees = 5, activate.ts = TRUE)
...
test_that("Does not crash when variable named 'none'", {
  dat <- data.frame(y = rbinom(100, 1, .5),
                    x = rbinom(100, 1, .5),
                    none = rbinom(100, 1, .5))
  rf <- ranger(data = dat, dependent.variable.name = "y", activate.ts = TRUE, block.size = 2, bootstrap.ts = 'nonoverlapping')
