...
res_boot<-  fit_forest2(block.size=25, bootstrap.ts="moving", activate.ts=T, param=param, data.pred=DataSim1)
fit_boot <- predict(res_boot$rf_fit, data=DataSim0)$prediction
forecast_boot <- predict(res_boot$rf_fit, data=DataSim1)$prediction

res_iid<-  fit_forest2(block.size=5, bootstrap.ts="circular", activate.ts=F, param=param, data.pred=DataSim1)
